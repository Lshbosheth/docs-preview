...
| 阶段 | 天 | 主题 | 状态 |
| --- | --- | --- | --- |
| 第一阶段：让消息在 Python 程序里流动 | Day 1 | 终端输入与学习记录 | ✅ 本批完成 |
| | Day 2 | 把用户输入变成消息字典 | ✅ 本批完成 |
| | Day 3 | 分类一条用户消息 | ✅ 本批完成 |
| | Day 4 | 把分类逻辑封装成函数 | ✅ 本批完成 |
| | Day 5 | 把单文件变成小项目 | ✅ 本批完成 |
| 第二阶段：保存消息并取得真实模型回复 | Day 6 | 把对话保存成 JSON | ✅ 已完成 |
| | Day 7 | 第一次调用 DeepSeek | ✅ 已完成 |
| | Day 8 | 用类封装模型客户端 | ✅ 已完成 |
| | Day 9 | 类型标注与结构化任务 | ✅ 已完成 |
| | Day 10 | 让 API 调用可恢复 | ✅ 已完成 |
| 第三阶段：从 LLM 应用走到小型 Agent | Day 11 | 用 LangChain 表达模型与消息 | ✅ 已完成 |
| | Day 12 | Prompt 与结构化输出 | ✅ 已完成 |
| | Day 13 | 让模型选择工具 | ✅ 已完成 |
| | Day 14 | 用 LangGraph 串起流程 | ✅ 已完成 |
| | Day 15 | 条件路由与会话状态 | ✅ 已完成 |

> 全部 15 篇已发布，可从课程首页或侧边栏逐篇访问。
